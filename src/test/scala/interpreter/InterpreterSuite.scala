...
class InterpreterSuite extends FunSuite {
  ...
  test("letIn evaluaation leads to replacement") {
  ...
  test("evaluate a tuple is equivalent to evaluate each of its element") {
    val tupleOfFac = Tuple(List(facApp(0), facApp(1), facApp(2)))
    val tupleOfFacEvaluated = Tuple(
      List(
        evaluate(facApp(0), 1000),
        evaluate(facApp(1), 1000),
        evaluate(facApp(2), 1000)
      )
    )
  ...
  test("select i-th element of tuple is evaluate the i-th element") {
    val tupleOfFac = Tuple(List(facApp(0), facApp(1), facApp(2)))
    assert(evaluate(TupleSelect(tupleOfFac, 1), 1000) === evaluate(facApp(1), 1000))
  }
}