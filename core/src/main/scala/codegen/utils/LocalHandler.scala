...
class LocalHandler(val rc : RunContext) {
  ...
  def get(id: Identifier): ParamDef =
    variables.getOrElse(translateId(id), rc.reporter.fatalError(s"Unknown variable $id"))
  ...
  def translateId(id: Identifier): String = id.asString(RunContext.printUniqueIds).replace("#", "_")
}
