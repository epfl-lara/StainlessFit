...
object Erasure {
  def erase(t: Tree)(implicit rc: RunContext): Tree = t match {
    ...
    case LetIn(_, t1, bind) => App(Lambda(None, erase(bind)), erase(t1))  //Let(None, erase(t1), erase(bind))
    ...
    case Unfold(t1, bind) => App(Lambda(None, erase(bind)), erase(t1))  //like a LetIn => erase(LetIn(None, t1, bind))
    case UnfoldPositive(t1, bind) => App(Lambda(None, erase(bind)), erase(t1)) //like a LetIn => erase(LetIn(None, t1, bind))
