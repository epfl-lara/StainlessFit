...
class ChooseEncoding(implicit val rc: RunContext) extends Phase[Unit] {
  ...
  def encode(path: Tree, n: Int, t: Tree): (Tree, Int) = t match {
    ...
    case FixWithDefault(ty, Bind(id, t), td, tf) =>
      val (nTy, n2) = encodeAnnot(n, ty)
      val (Seq(nT, nTd, nTf), n3) = encode(path, n2, Seq(t, td, tf))
      (FixWithDefault(nTy, Bind(id, nT), nTd, nTf), n3)
    ...
    case Ascribe(t, ty) =>
      val (nt, n2) = encode(path, n, t)
      val (nTy, nn) = encodeAnnot(n2, ty)
      (Ascribe(nt, nTy), nn)
  }
}
